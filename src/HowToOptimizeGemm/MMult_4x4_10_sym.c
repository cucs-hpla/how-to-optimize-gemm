...
void AddDot4x4( int, double *, double *, double*, int, int, double *, int, double *, int );
  ...
  for ( j=0; j<n; j+=4 ){        /* Loop over the columns of C, unrolled by 4 */
    for ( i=0; i<m; i+=4 ){        /* Loop over the rows of C */
	 ...
	 one routine (four inner products) */
      ...
      AddDot4x4( k, &A( i,0 ), &A(i,i), &A(i+2,i+2), i, lda, &B( 0,j ), ldb, &C( i,j ), ldc );
    }
  }
}
...
void AddDot4x4( int k, double *a, double *a0, double *a2, int i, int lda,  double *b, int ldb, double *c, int ldc )
{
  ...
  for ( p=0; p<k; p++ ){
    if(p<i){
        ...
        a_0p_a_1p_vreg.v = _mm_load_pd( (double *) &a0[p-i] );
        a_2p_a_3p_vreg.v = _mm_load_pd( (double *) &a2[p-i-2] );
    }
